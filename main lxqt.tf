...
resource "google_compute_disk" "lxqt_data_disks" {
  count = 3
...
# 3. VM Instances Loop
resource "google_compute_instance" "lxqt_debian_nodes" {
  count        = 3
  ...
  attached_disk {
    source      = google_compute_disk.lxqt_data_disks[count.index].self_link
